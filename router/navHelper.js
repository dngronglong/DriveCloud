...
export default {
	...
	methods: {
		...
		relaunchToHome() {
			uni.reLaunch({
				url: "/pages/index/index?target=home"
			})
		},
		relaunchToFiles() {
			uni.reLaunch({
				url: "/pages/index/index?target=files"
			})
		},
		relaunchToMine() {
			uni.reLaunch({
				url: "/pages/index/index?target=mine"
			})
		},
		relaunchToShare() {
			uni.reLaunch({
				url: "/pages/index/index?target=share"
			})
		},
		relaunchToCurrentTab() {
			...
			if (currentTab === 0) {
				...
				this.relaunchToFiles()
			} else if (currentTab === 2) {
				this.relaunchToShare()
